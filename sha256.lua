return function(input)
	local bit = bit32

	local H = {
		0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
		0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
	}

	local K = {
		0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
		0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
		0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
		0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
		0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
		0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
		0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
		0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
	}

	local function preprocess(msg)
		local len = #msg * 8
		msg = msg .. string.char(0x80)
		local current_length = #msg
		local mod = current_length % 64
		
		if mod ~= 56 then
			msg = msg .. string.rep(string.char(0), 56 - mod)
		end
		
		msg = msg .. string.char(bit.band(bit.rshift(len, 56), 0xff), bit.band(bit.rshift(len, 48), 0xff), bit.band(bit.rshift(len, 40), 0xff), bit.band(bit.rshift(len, 32), 0xff), bit.band(bit.rshift(len, 24), 0xff), bit.band(bit.rshift(len, 16), 0xff), bit.band(bit.rshift(len, 8), 0xff), bit.band(len, 0xff))
		
		return msg
	end

	local function chunk(msg, i)
		local W = {}
		
		for j = 1, 64 do
			if j <= 16 then
				W[j] = string.byte(msg, i + (j - 1) * 4) * 0x1000000 + string.byte(msg, i + (j - 1) * 4 + 1) * 0x10000 + string.byte(msg, i + (j - 1) * 4 + 2) * 0x100 + string.byte(msg, i + (j - 1) * 4 + 3)
			else
				local s0 = bit.bxor(bit.rrotate(W[j - 15], 7), bit.rrotate(W[j - 15], 18), bit.rshift(W[j - 15], 3))
				local s1 = bit.bxor(bit.rrotate(W[j - 2], 17), bit.rrotate(W[j - 2], 19), bit.rshift(W[j - 2], 10))
				W[j] = (W[j - 16] + s0 + W[j - 7] + s1) % 0x100000000
			end
		end
		
		local a, b, c, d, e, f, g, h = table.unpack(H)
		
		for j = 1, 64 do
			local S1 = bit.bxor(bit.rrotate(e, 6), bit.rrotate(e, 11), bit.rrotate(e, 25))
			local ch = bit.bxor(bit.band(e, f), bit.band(bit.bnot(e), g))
			local temp1 = (h + S1 + ch + K[j] + W[j]) % 0x100000000
			local S0 = bit.bxor(bit.rrotate(a, 2), bit.rrotate(a, 13), bit.rrotate(a, 22))
			local maj = bit.bxor(bit.band(a, b), bit.band(a, c), bit.band(b, c))
			local temp2 = (S0 + maj) % 0x100000000
			
			h = g
			g = f
			f = e
			e = (d + temp1) % 0x100000000
			d = c
			c = b
			b = a
			a = (temp1 + temp2) % 0x100000000
		end
		
		H[1] = (H[1] + a) % 0x100000000
		H[2] = (H[2] + b) % 0x100000000
		H[3] = (H[3] + c) % 0x100000000
		H[4] = (H[4] + d) % 0x100000000
		H[5] = (H[5] + e) % 0x100000000
		H[6] = (H[6] + f) % 0x100000000
		H[7] = (H[7] + g) % 0x100000000
		H[8] = (H[8] + h) % 0x100000000
	end
	
	local msg = preprocess(input)
	
	for i = 1, #msg, 64 do
		chunk(msg, i)
	end
	
	return ('%08x'):rep(8):format(table.unpack(H))
end
